...
export function TrackingTimeline({ updates }: TrackingTimelineProps) {
  ...
  const initialTimestamp = Math.min(
    ...history.map((update) => new Date(update.timestamp).getTime())
  );
  ...
  return (
    <div className="tracking-history-table">
      ...
      {sortedUpdates.map((update, index) => {
        ...
        const isInitial = new Date(update.timestamp).getTime() === initialTimestamp;

        return (
          <div key={update.id || index} className="tracking-history-row">
            <span>{isInitial ? "INIT" : formatDate(update.timestamp)}</span>
